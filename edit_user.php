...
<?php
  ...
  if(!$e_user){
    $session->msg("d","Usuario no se encuentra en la base de datos.");
    redirect('users.php');
  }
?>
...
//Update User basic info
  if(isset($_POST['update'])) {
    $req_fields = array('name','username','level', 'store');
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
        $store   = remove_junk($db->escape($_POST['store']));
            $sql = "UPDATE users SET name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}',store_id='{$store}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
              ...
              <label for="store">Almacen</label>
                <select class="form-control" name="store">
                  <?php foreach ($stores as $store ):?>
                   <option <?php if($store['id'] === $e_user['store_id']) echo 'selected="selected"';?> value="<?php echo $store['id'];?>"><?php echo ucwords($store['name']);?></option>
                <?php endforeach;?>
                </select>
            </div>
            <div class="form-group">
